...
{
    /**
     * Download the overall student ranking in Excel format.
     *
     * @param  int  $id
     * @param  string  $slug
     * @param  int  $form_id
     ...
     */
    public function downloadOverallRankingExcel(Request $request, $slug, $form_id)
    {
      // Retrieve the exam ID
      if (is_numeric($request->id)) {
        $examId = (int) $request->id;
      } else {
        // Handle case where ID is not numeric (if applicable)
      ...
      foreach ($students as $student) {
        ...
        if ($subject1Marks > 0 && $subject2Marks > 0) {
          ...
          $studentMeans[] = [
            'rank' => '', // Placeholder for rank (to be calculated later)
            'adm' => $student->adm,
            'name' => $student->name,
      ...
      // Pass exam ID to the export class and trigger the download
      return Excel::download(new OverallStudentRankingExport($examId, $studentMeans, $gradingSystem), 'overall_student_ranking.xlsx');
    }
