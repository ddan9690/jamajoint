...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/marksheet/school/{schoolId}/{slug}', [MarkSheetController::class, 'generate'])->name('marksheet');
...
// grading system manager
    ...
    Route::get('/excel-download/stream-ranking/{id}/{form_id}/{slug}', [ExcelDownloadController::class, 'streamRanking'])
        ...
        Route::get('/generate-overall-ranking-pdf/{id}/{slug}/{form_id}', [OverallRankingController::class, 'downloadOverallRankingPdf'])
        ->name('pdf.overall-student-ranking');
    Route::get('/download-pdf/{filename}', [OverallRankingController::class, 'downloadGeneratedPdf'])
        ->name('download.pdf');
